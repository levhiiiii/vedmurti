...
exports.generateAutomaticPayouts = functions.pubsub.schedule('0 0 2,12,22 * *').timeZone('Asia/Kolkata').onRun(async (context) => {
  try {
    ...
    for (const userDoc of usersSnapshot.docs) {
      ...
      try {
        ...
        if (bankAccountsSnapshot.empty) {
          console.log(`No primary bank account found for user ${userId}`);
          continue;
        }
        ...
        if (totalIncome > 0) {
          ...
          console.log(`Generated payout for user ${userId}: ₹${payoutData.payoutAmount}`);
        }
      } catch (error) {
    ...
    console.log(`Automatic payout generation completed: ${totalPayouts} payouts, ₹${totalAmount} total`);
    
    return { success: true, totalPayouts, totalAmount };
  } catch (error) {
    console.error('Error in automatic payout generation:', error);
    throw error;
  }
});
...
exports.manualPayoutGeneration = functions.https.onRequest(async (req, res) => {
  try {
    ...
    console.error('Error in manual payout generation:', error);
    res.status(500).json({
      success: false,
      error: error.message
    });
