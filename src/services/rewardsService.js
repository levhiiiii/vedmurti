...
export class RewardsService {
  ...
  static async calculateAndDistributeRewards() {
    try {
      ...
      await batch.commit();
      console.log(`Distributed ₹${totalRewardsDistributed} in rewards to ${mlmUsersSnapshot.size} users`);
  ...
  static async getFestivalRewards(userId, pairsCount, businessVolume) {
    try {
      ...
      console.error('Error calculating festival rewards:', error);
      return [];
    }
  }
  ...
  static async adjustUserReward(userId, adjustment, reason, adminId) {
    try {
      ...
      console.error('Error adjusting user reward:', error);
      throw error;
    }
  }
}
