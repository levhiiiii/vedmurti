...
export class ScheduledService {
  ...
  static async resetDailyCycles() {
    try {
      console.log('Resetting daily cycles...');
      ...
      if (resetCount > 0) {
        await batch.commit();
        console.log(`Reset daily cycles for ${resetCount} users`);
      }
  ...
  static async processPerformanceRewards() {
    try {
      console.log('Processing performance rewards...');
      ...
      if (rewardsProcessed > 0) {
        await batch.commit();
        console.log(`Processed ${rewardsProcessed} performance rewards totaling ₹${totalRewardAmount}`);
      }
      
      return { rewardsProcessed, totalRewardAmount };
    } catch (error) {
      console.error('Error processing performance rewards:', error);
      throw error;
    }
  }
  ...
  static async cleanupOldRecords() {
    try {
      ...
      if (deletedRecords > 0) {
        await batch.commit();
        console.log(`Cleaned up ${deletedRecords} old income records`);
      }
  ...
  static initializeScheduler() {
    console.log('Initializing MLM scheduler...');
